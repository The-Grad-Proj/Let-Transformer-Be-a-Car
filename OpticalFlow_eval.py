...
def main():
    ...
    csv_file=f"{DATASET_PATH}/CH2_final_evaluation.csv"
    ...
    for index, row in camera_csv.iterrows():
        file_name = camera_csv.loc[index, 'frame_id']
    ...
    for index, row in camera_csv.iterrows():
        ...
        if previous_file_name is None:
            continue
        current_path = os.path.join(DATASET_PATH, "center", current_file_name)
        previous_path= os.path.join(DATASET_PATH, "center", previous_file_name)
